...
An Amiga game I wrote back in 1990! Originaly intended to be published by the German Amiga Magazin "Kickstart" but that never happend.

![Menu](https://github.com/LutzGrosshennig/amiga-brain-braiser/blob/main/Images/Menu.jpg)
![Game](https://github.com/LutzGrosshennig/amiga-brain-braiser/blob/main/Images/Game.jpg)
...
A 12 × 13 tile playing field is generated and filled with random numbers between -9 and +9, plus two kinds of special squares (zero and X). 

Player 1 can move horizontally on the field, whereas Player 2 can only move vertically. The aim of the game is to achieve the highest possible score. The simplest way to do this is to always occupy the highest-value tile in the current row or column, but if you do that you will lose very quickly, because your opponent can then plan more than five moves ahead and lure you into a trap. A running clock limits turn time, so with short time settings you must act very quickly.

Additional special squares can be used strategically. One type are the exchange squares (marked with a large X), which swap the score of Player 1 and Player 2 — allowing a player who is behind to "steal" their opponent’s hard-earned points. The other type are Zero squares (square value is 0); these invert the signs of all numbers on the board (a -9 becomes 9, etc.). If a player's clock runs out, the other player receives a 15-point bonus and gets the next move. If a player can no longer move because their row or column is completely filled, their opponent receives a 25-point bonus and the game ends.

# The Setup Menu

In the setup menu, which appears before each new game, you can choose whether to play against the computer or prefer a human opponent. You can also set the control type separately for each player. In a string gadget you can conveniently set the maximum move duration (default 500 / corresponds to 10 seconds). If you want to set the clock yourself, please note: the clock is decremented 50 times per second on a PAL system. So you must multiply the number of seconds by 50 to get the correct value for the clock. In addition, you can set the number of joysticks available.

# Controls

Player 1 controls the cursor with the "A", "D" and the spacebar, or via a joystick in Port 2. Player 2 controls the cursor with the up and down arrow keys and the spacebar, or via a joystick in Port 2. If two joysticks are present and configured, Player 1 uses the joystick in Port 2 and Player 2 uses the joystick in Port 1.
